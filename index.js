...
async function startXliconBot() {
    ...
    XliconBotInc.ev.on('group-participants.update', async (update) => {
        ...
        for (let participant of participants) {
            let isWelcome = update.action === 'add';
            let isBye = update.action === 'remove';

            // Auto-Welcome
            if (isWelcome) {
                let userProfilePic = await XliconBotInc.profilePictureUrl(participant, 'image').catch(() => null);
                let groupProfilePic = await XliconBotInc.profilePictureUrl(id, 'image');
                let picture = userProfilePic || groupProfilePic;
                
                await XliconBotInc.sendMessage(id, {
                    text: `Selamat datang @${participant.split('@')[0]} semoga betah di grup dan jangan lupa patuhi rules yaa`,
                    mentions: [participant]
                });

                if (picture) {
                    const imageBuffer = await getBuffer(picture);
                    await XliconBotInc.sendMessage(id, { image: imageBuffer, caption: `Welcome @${participant.split('@')[0]}`, mentions: [participant] });
                }
            }
            ...
            if (isBye) {
                await XliconBotInc.sendMessage(id, {
                    text: `Selamat jalan @${participant.split('@')[0]} semoga tenang disana`,
                    mentions: [participant]
                });
            }
        }
